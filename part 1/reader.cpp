...
int main() {
    ...
    if (file_mapping != nullptr) {
        for (size_t i = 0; i < operation_count; ++i) {
            size_t page = WaitForMultipleObjects(written_page_semaphores.size(), written_page_semaphores.data(), false, INFINITE);
            logger.logging("OBTAIN WRITTEN_PAGE_SEMAPHORE FOR PAGE " + std::to_string(page), std_out);

            WaitForSingleObject(race_mutex, INFINITE);
            logger.logging("OBTAIN RACE_MUTEX", std_out);

            logger.logging("START READING", std_out);
            SleepEx(rand() % 1000 + 500, false);
            logger.logging("END READING", std_out);
            ...
            if (ReleaseSemaphore(read_page_semaphores[page], 1 , nullptr))
                logger.logging("RELEASE READ_PAGE_SEMAPHORE FOR PAGE " + std::to_string(page), std_out);
            else
                logger.logging("CODE " + std::to_string(GetLastError()), std_out);
        }
    }
    else
        logger.logging("MAPPING READ FAIL", std_out);
